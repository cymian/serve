/**
 Exports the `createRequestGuard()` factory and its `RequestGuardOptions` and
 `RequestGuard` types. A local server runs the guard ahead of its routes to
 admit the page that server sent and programs on this machine, and to refuse
 everything else before a route sees it.

 - **the binding it assumes**: every check here takes the server to be bound to
   this machine. A server reachable from anywhere can't tell its own caller
   from a stranger, and none of this substitutes for authentication past
   loopback
 - **which checks apply**: a static server needs the host and cross-site checks;
   the mutation and client-header checks apply when a route mutates or exposes
   something
 - **what it imports**: only `getLanAddresses()`, so a guard consumer doesn't
   typecheck the static server or resolve its dependencies
...
/*
 ...
 - allow a top-level navigation from elsewhere again if something needs one,
   ...
   - require `Sec-Fetch-Dest: document`, the value for the tab itself; embeds
     report `iframe`, `frame`, `object`, or `embed` instead
*/
...
export interface RequestGuardOptions {
  ...
  */
  ...
  /**
   ...
   - its value does not matter; a header outside that set requires a successful
      preflight, and neither a `no-cors` send nor a simple cross-origin `POST`
      can include one
  */
  clientHeader?: string;
  /**
...
/** `Sec-Fetch-Site` values that don't put a request on another origin's behalf. */
...
// - `same-site` is deliberately absent: a site ignores the port, so admitting
//   it would admit every other server on this machine
// - browsers set `Sec-Fetch-*` only for a potentially trustworthy URL, so a
//   LAN address over plain `http` arrives with none of them
...
export function createRequestGuard(options: RequestGuardOptions): RequestGuard {
  ...
  // - only :80 gets the bare origin, since every origin here uses `http`
  ...
  return (request) => {
    ...
    // - an absent `Origin` means it wasn't a browser: the fetch spec appends
    //   one to every request whose method isn't `GET` or `HEAD`, leaving curl
    //   and scripts, which the loopback binding already scopes to this machine
...
/**
 Returns true if the request's `Sec-Fetch-Site` is one a local server may
 answer.
 ...
 - an embed is refused too, which the absent CORS header would not have done:
...
/** Returns the 403 a failed check earns, naming the check for DevTools. */
